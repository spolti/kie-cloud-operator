...
func TestRHPAMDashbuilderDefaultEnvironment(t *testing.T) {
	...
	assert.Equal(t, *dashLimAndReq.Limits.Cpu(), cr.Status.Applied.Objects.Dashbuilder.Resources.Limits[corev1.ResourceCPU])
	assert.Equal(t, *dashLimAndReq.Requests.Cpu(), cr.Status.Applied.Objects.Dashbuilder.Resources.Requests[corev1.ResourceCPU])
	assert.Equal(t, *dashLimAndReq.Limits.Memory(), cr.Status.Applied.Objects.Dashbuilder.Resources.Limits[corev1.ResourceMemory])
	assert.Equal(t, *dashLimAndReq.Requests.Memory(), cr.Status.Applied.Objects.Dashbuilder.Resources.Requests[corev1.ResourceMemory])
...
func TestCRServerCPULimitAndRequestUsingMilicores(t *testing.T) {
	...
	env = ConsolidateObjects(env, cr)
	fmt.Printf("Defined +%v", cr.Spec.Objects.Servers[0].Resources)
	fmt.Printf("\nApplied +%v\n", cr.Status.Applied.Objects.Servers[0].Resources)
	fmt.Printf("DeploymentConfig +%v\n", env.Servers[0].DeploymentConfigs[0].Spec.Template.Spec.Containers[0].Resources)
